...
function sortairfields()
    -- Set your reference airfield name here (must exist in mission)
    local referenceAirfield = "Rayak"
    local refAirbase = AIRBASE:FindByName(referenceAirfield)
    if not refAirbase then
        env.info("Reference airfield not found: " .. referenceAirfield)
        return
    end
...
function Spawn_Near_airbase(GroupTemplate, airfieldName, Inner, Outer, Patrol)
    ...
    if WarehouseFound then
        ...
        Spawnpoint = WarehouseZone:GetRandomCoordinate(200, 1000, land.SurfaceType.ROAD)
...
function SpawnWarehouse(airfieldName, warehouseName, coalitionSide)
    ...
    repeat
        Spawnpoint = SpawnZone:GetRandomCoordinate(250, 1000, land.SurfaceType.ROAD)
...
-- Helper to spawn a group near the warehouse or fallback zone
    local function SpawnGroupNearZone(GroupTemplate, Patrol, repeatCount)
        ...
        local function trySpawn(attempt)
            attempt = attempt or 1
            if attempt > maxAttempts then
                env.info("SpawnGroupNearZone: Max attempts reached for " .. tostring(GroupTemplate))
                return
            end
            ...
            if WarehouseZone then
                Spawnpoint = SpawnZone:GetRandomCoordinate(300, 1500, land.SurfaceType.LAND)
            else
                Spawnpoint = SpawnZone:GetRandomCoordinate(MinDistance, MaxDistance, land.SurfaceType.LAND)
            end

            env.info("Spawning "..GroupTemplate.." with name "..GroupName.." (Attempt "..attempt..")")
            local Group_Spawn = SPAWN:NewWithAlias(GroupTemplate, GroupName)
            Group_Spawn:InitPositionCoordinate(Spawnpoint)
            Group_Spawn:OnSpawnGroup(function(spawnedGroup)
                -- Check if group is on runway
                if AIRBASE:CheckOnRunWay(spawnedGroup, 50, true) then
                    env.info("Spawned group was on runway, deleting and retrying: " .. GroupName)
                    -- Try again
                    trySpawn(attempt + 1)
                else
                    if Patrol then
        ...
        if parkingData.aircraftParkingCount > 80 then
            SpawnGroupNearZone(Group_Blue_SAM_Site, false, 1)
        end

        local blueSAMTemplates = {Group_Blue_SAM1, Group_Blue_SAM2, Group_Blue_SAM3, Group_Blue_SAM4}

        for i = 1, 4 do  -- 2 spawns
            local randomIndex = math.random(1, #blueSAMTemplates)
            local chosenTemplate = blueSAMTemplates[randomIndex]
...
-- Helper to spawn a group near the warehouse or fallback zone
    local function SpawnGroupNearZone(GroupTemplate, Patrol, repeatCount)
        ...
        if parkingData.aircraftParkingCount > 100 then
            SpawnGroupNearZone(Group_Red_SAM_Site, false,1)
        end

        local redSAMTemplates = {Group_Red_SAM1, Group_Red_SAM2, Group_Red_SAM3, Group_Red_SAM4}

        for i = 1, 4 do  -- 2 spawns
            local randomIndex = math.random(1, #redSAMTemplates)
...
function CreateBlueAirwing(warehouse, airwingName, airfieldName)
    ...
    --setup arrays of templates for spawning all groups defined in 04_WoC_Groups_MW.lua
    local Blue_Fighter_Choices = {Blue_Fighter1, Blue_Fighter2, Blue_Fighter3}
    local Blue_LT_Fighter_Choices = {Blue_LT_Fighter1, Blue_LT_Fighter2, Blue_LT_Fighter3}
    local Blue_Attack_Choices = {Blue_Attack1, Blue_Attack2, Blue_Attack3}
    local Blue_Helo_Choices = {Blue_Helo1, Blue_Helo2, Blue_Helo3}
    local Blue_AttackHelo_Choices = {Blue_AttackHelo1, Blue_AttackHelo2, Blue_AttackHelo3}
...
if parkingData.aircraftParkingCount > 10 then
        ...
        local Blue_Fighter = pickRandom(Blue_Fighter_Choices)
        ...
        local Blue_LT_Fighter = pickRandom(Blue_LT_Fighter_Choices)
        local SQN2 = SQUADRON:New(Blue_LT_Fighter, 2, "Blue Light Fighter Squadron "..airfieldName)
        SQN2:AddMissionCapability({AUFTRAG.Type.GCICAP, AUFTRAG.Type.CAP, AUFTRAG.Type.INTERCEPT, AUFTRAG.Type.ESCORT, AUFTRAG.Type.RECON,AUFTRAG.Type.CAS, AUFTRAG.Type.CASENHANCED,AUFTRAG.Type.BAI, AUFTRAG.Type.BOMBING,AUFTRAG.Type.SEAD},70)
        ...
        local Blue_Attack = pickRandom(Blue_Attack_Choices)
        local SQN3 = SQUADRON:New(Blue_Attack, 2, "Blue Attack Squadron "..airfieldName)
        SQN3:AddMissionCapability({AUFTRAG.Type.CAS, AUFTRAG.Type.CASENHANCED,AUFTRAG.Type.BAI, AUFTRAG.Type.BOMBING},80)
       ...
       Blue_payload_Attack_SEAD =airwing:NewPayload(GROUP:FindByName(Blue_Attack.."_SEAD"), 2, {AUFTRAG.Type.CAS, AUFTRAG.Type.CASENHANCED,AUFTRAG.Type.BAI, AUFTRAG.Type.BOMBING},80)
    ...
    if parkingData.heliParkingCount > 1 or parkingData.aircraftParkingCount > 1 then
        local Blue_Helo = pickRandom(Blue_Helo_Choices)
        local SQN4 = SQUADRON:New(Blue_Helo, 4, "Blue Transport Helo Squadron "..airfieldName)
        ...
        local Blue_AttackHelo = pickRandom(Blue_AttackHelo_Choices)
        local SQN5 = SQUADRON:New(Blue_AttackHelo, 4, "Blue Attack Helo Squadron "..airfieldName)
       ...
       Blue_payload_helo_CAS = airwing:NewPayload(GROUP:FindByName(Blue_Helo.."_CAS"), 4, {AUFTRAG.Type.TROOPTRANSPORT, AUFTRAG.Type.CARGOTRANSPORT, AUFTRAG.Type.RECON, AUFTRAG.Type.OPSTRANSPORT},80)
       Blue_payload_Attackhelo_CAS = airwing:NewPayload(GROUP:FindByName(Blue_AttackHelo.."_CAS"), 4, {AUFTRAG.Type.CAS, AUFTRAG.Type.BAI,AUFTRAG.Type.ESCORT},50)
    ...
    Brigade:SetSpawnZone(airbase:GetZone())
        -- TPz Fuchs platoon.
    local platoonAPC=PLATOON:New(Group_Blue_APC, 10, "Blue Motorised Platoon "..airfieldName)
    platoonAPC:AddMissionCapability(APC_MissionSet, 60):SetAttribute(GROUP.Attribute.GROUND_APC)
        -- Mechanised platoon
    local platoonMECH=PLATOON:New(Group_Blue_Mech, 10,"Blue Mechanised Platoon "..airfieldName)
    ...
    platoonMECH:AddWeaponRange(UTILS.KiloMetersToNM(0.5), UTILS.KiloMetersToNM(20))
        -- Armoured platoon
    local platoonArmoured =PLATOON:New(Group_Blue_Armoured, 10,"Blue Armoured Platoon "..airfieldName)
    ...
    local platoonLogi=PLATOON:New(Group_Blue_Truck, 10, "Blue Logistics Platoon "..airfieldName)
    ...
    --platoonINF:AddMissionCapability({AUFTRAG.Type.GROUNDATTACK, AUFTRAG.Type.ONGUARD}, 50)
        -- mobile SAM
    local platoonSAM1=PLATOON:New(Group_Blue_SAM1, 2, "Blue SAM Platoon 1 "..airfieldName)
    platoonSAM1:AddMissionCapability({AUFTRAG.Type.AIRDEFENSE}, 100)
    local platoonSAM2=PLATOON:New(Group_Blue_SAM2, 2, "Blue SAM Platoon 2"..airfieldName)
    platoonSAM2:AddMissionCapability({AUFTRAG.Type.AIRDEFENSE}, 90)
    local platoonSAM3=PLATOON:New(Group_Blue_SAM3, 2, "Blue SAM Platoon 3"..airfieldName)
    platoonSAM3:AddMissionCapability({AUFTRAG.Type.AIRDEFENSE}, 80)
    local platoonSAM4=PLATOON:New(Group_Blue_SAM4, 2, "Blue SAM Platoon 4"..airfieldName)
    platoonSAM4:AddMissionCapability({AUFTRAG.Type.AIRDEFENSE}, 70)
    ...
    Brigade:AddPlatoon(platoonSAM1)
    Brigade:AddPlatoon(platoonSAM2)
    Brigade:AddPlatoon(platoonSAM3)
    Brigade:AddPlatoon(platoonSAM4)
...
function CreateRedAirwing(warehouse, airwingName, airfieldName)
    ...
    -- Arrays of possible templates for each squadron
    local Red_Fighter_Choices = {Red_Fighter1, Red_Fighter2, Red_Fighter3}
    local Red_LT_Fighter_Choices = {Red_LT_Fighter1, Red_LT_Fighter2, Red_LT_Fighter3}
    local Red_Attack_Choices = {Red_Attack1, Red_Attack2, Red_Attack3}
    local Red_Helo_Choices = {Red_Helo1, Red_Helo2, Red_Helo3}
    local Red_AttackHelo_Choices = {Red_AttackHelo1, Red_AttackHelo2, Red_AttackHelo3}
    ...
    env.info(airwingName.. " added to Red Airwing list")
    ...
    if parkingData.aircraftParkingCount > 10 then
        local Red_Fighter = pickRandom(Red_Fighter_Choices)
        ...
        local Red_Attack = pickRandom(Red_Attack_Choices)
        ...
        local Red_LT_Fighter = pickRandom(Red_LT_Fighter_Choices)
        ...
        Red_payload_LtFighter_AA = airwing:NewPayload(GROUP:FindByName(Red_LT_Fighter.."_AA"), 2, {AUFTRAG.Type.GCICAP, AUFTRAG.Type.CAP, AUFTRAG.Type.INTERCEPT, AUFTRAG.Type.ESCORT, AUFTRAG.Type.RECON},70)
        Red_payload_Attack_SEAD = airwing:NewPayload(GROUP:FindByName(Red_Attack.."_SEAD"), 2, {AUFTRAG.Type.SEAD},90)
        Red_payload_Attack_CAS = airwing:NewPayload(GROUP:FindByName(Red_Attack.."_CAS"), 2,{AUFTRAG.Type.CAS, AUFTRAG.Type.CASENHANCED,AUFTRAG.Type.BAI, AUFTRAG.Type.BOMBING}, 80)
        airwing:AddSquadron(SQN1)
        airwing:AddSquadron(SQN2)
        airwing:AddSquadron(SQN3)
    ...
    if parkingData.heliParkingCount > 1 or parkingData.aircraftParkingCount > 1 then
        local Red_Helo = pickRandom(Red_Helo_Choices)
        local SQN4 = SQUADRON:New(Red_Helo, 4, "Red Transport Helo Squadron "..airfieldName)
        ...
        local Red_AttackHelo = pickRandom(Red_AttackHelo_Choices)
        local SQN5 = SQUADRON:New(Red_AttackHelo, 4, "Red Attack Helo Squadron "..airfieldName)
        SQN5:AddMissionCapability({AUFTRAG.Type.CAS, AUFTRAG.Type.BAI,AUFTRAG.Type.ESCORT}):SetAttribute(GROUP.Attribute.AIR_TRANSPORTHELO)
        SQN5:SetDespawnAfterLanding(true)
        SQN5:SetDespawnAfterHolding(true)
        SQN5:SetMissionRange(40)
        ...
        Red_payload_helo_CAS = airwing:NewPayload(GROUP:FindByName(Red_Helo.."_CAS"), 4, {AUFTRAG.Type.TROOPTRANSPORT, AUFTRAG.Type.CARGOTRANSPORT, AUFTRAG.Type.RECON, AUFTRAG.Type.OPSTRANSPORT},80)
        Red_payload_Attackhelo_CAS = airwing:NewPayload(GROUP:FindByName(Red_AttackHelo.."_CAS"), 4, {AUFTRAG.Type.CAS, AUFTRAG.Type.BAI,AUFTRAG.Type.ESCORT},50)
        Red_payload_Attackhelo_Trans = airwing:NewPayload(GROUP:FindByName(Red_AttackHelo.."_Trans"), 4, {AUFTRAG.Type.CAS, AUFTRAG.Type.BAI,AUFTRAG.Type.ESCORT},50)
    ...
    Brigade:SetSpawnZone(airbase:GetZone())
       ...
       --platoonINF:AddMissionCapability({AUFTRAG.Type.GROUNDATTACK, AUFTRAG.Type.ONGUARD}, 50)
            -- mobile SAM
        local platoonSAM1=PLATOON:New(Group_Red_SAM1, 1,  "Red SAM Platoon 1"..airfieldName)
        platoonSAM1:AddMissionCapability({AUFTRAG.Type.AIRDEFENSE}, 100)
        local platoonSAM2=PLATOON:New(Group_Blue_SAM2, 2, "Red SAM Platoon 2 "..airfieldName)
        platoonSAM2:AddMissionCapability({AUFTRAG.Type.AIRDEFENSE}, 90)
        local platoonSAM3=PLATOON:New(Group_Blue_SAM3, 2, "Red SAM Platoon 3"..airfieldName)
        platoonSAM3:AddMissionCapability({AUFTRAG.Type.AIRDEFENSE}, 80)
        local platoonSAM4=PLATOON:New(Group_Blue_SAM4, 2, "Red SAM Platoon 4"..airfieldName)
        platoonSAM4:AddMissionCapability({AUFTRAG.Type.AIRDEFENSE}, 70)
...
local CapZone8 = ZONE:FindByName("CAP_Zone_Mid-1")
local CapZone9 = ZONE:FindByName("CAP_Zone_Mid-2")
local CapZone10 = ZONE:FindByName("CAP_Zone_Mid-3")
local CapZone11 = ZONE:FindByName("CAP_Zone_SE-1")
local CapZone12 = ZONE:FindByName("CAP_Zone_SE-2")
local CapZone13 = ZONE:FindByName("CAP_Zone_SE-3")
local CapZone14 = ZONE:FindByName("CAP_Zone_SE-4")
local CapZone15 = ZONE:FindByName("CAP_Zone_SW-1")
local CapZone16 = ZONE:FindByName("CAP_Zone_SW-2")
local CapZone17 = ZONE:FindByName("CAP_Zone_SW-3")
...
function CreateBlueChief()
    ...
    BlueChief:AddCapZone(CapZone11,26000,400,180,25)
    BlueChief:AddCapZone(CapZone12,26000,400,180,25)
    BlueChief:AddCapZone(CapZone13,26000,400,180,25)
    BlueChief:AddCapZone(CapZone14,26000,400,180,25)
    BlueChief:AddBorderZone(CapZone8)
    BlueChief:AddBorderZone(CapZone9)
    BlueChief:AddBorderZone(CapZone10)
    BlueChief:AddBorderZone(CapZone11)
    BlueChief:AddBorderZone(CapZone12)
    BlueChief:AddBorderZone(CapZone13)
    BlueChief:AddBorderZone(CapZone14)
    BlueChief:AddConflictZone(CapZone1)
    BlueChief:AddConflictZone(CapZone2)
    BlueChief:AddConflictZone(CapZone3)
...
function CreateRedChief()
    ...
    RedChief:AddConflictZone(CapZone8)
    RedChief:AddConflictZone(CapZone9)
    RedChief:AddConflictZone(CapZone10)
    RedChief:AddConflictZone(CapZone11)
    RedChief:AddConflictZone(CapZone12)
    RedChief:AddConflictZone(CapZone13)
    RedChief:AddConflictZone(CapZone14)
    ...
    RedChief:AddBorderZone(CapZone7)
...
function PlayerTaskingBlue()
    ...
    BlueTaskManagerA2G:AddAcceptZone(CapZone1 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone2 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone3 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone4 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone5 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone6 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone7 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone8 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone9 )
    BlueTaskManagerA2G:AddAcceptZone(CapZone10)
    BlueTaskManagerA2G:AddAcceptZone(CapZone11)
    BlueTaskManagerA2G:AddAcceptZone(CapZone12)
    BlueTaskManagerA2G:AddAcceptZone(CapZone13)
    BlueTaskManagerA2G:AddAcceptZone(CapZone14)
...
function PlayerTaskingRed()
  ...
  --_SETTINGS:SetA2G_BR()
    ...
    -- Add accept- and reject-zones for detection
    -- Accept zones are handy to limit e.g. the engagement to a certain zone. The example is a round, mission editor created zone named "AcceptZone"
    RedTaskManagerA2G:AddAcceptZone(CapZone1 )
    RedTaskManagerA2G:AddAcceptZone(CapZone2 )
    RedTaskManagerA2G:AddAcceptZone(CapZone3 )
    RedTaskManagerA2G:AddAcceptZone(CapZone4 )
    RedTaskManagerA2G:AddAcceptZone(CapZone5 )
    RedTaskManagerA2G:AddAcceptZone(CapZone6 )
    RedTaskManagerA2G:AddAcceptZone(CapZone7 )
    RedTaskManagerA2G:AddAcceptZone(CapZone8 )
    RedTaskManagerA2G:AddAcceptZone(CapZone9 )
    RedTaskManagerA2G:AddAcceptZone(CapZone10)
    RedTaskManagerA2G:AddAcceptZone(CapZone11)
    RedTaskManagerA2G:AddAcceptZone(CapZone12)
    RedTaskManagerA2G:AddAcceptZone(CapZone13)
    RedTaskManagerA2G:AddAcceptZone(CapZone14)
...
function BlueOpsCTLD()
    ...
    SETTINGS:SetPlayerMenuOff()
       ...
       -- add infantry unit called "Anti-Tank" using templates "AA" and "AA"", of type TROOP with size 4. No weight. We only have 2 in stock:
          ...
          Blue_ctld:AddTroopsCargo("APC",{Group_Blue_APC},CTLD_CARGO.Enum.TROOPS,4,nil)
...
local function ProduceAirwing(warehouseName, airwing, Coalition)
    ...
    if factory and factory:IsAlive() then
        ...
        if Coalition == "Blue" then
            ...
            IncreaseIfBelowLimit(Blue_payload_Attack_SEAD)
            IncreaseIfBelowLimit(Blue_payload_helo_Trans)
            IncreaseIfBelowLimit(Blue_payload_helo_CAS)
            IncreaseIfBelowLimit(Blue_payload_Attackhelo_CAS)
        ...
        elseif Coalition == "Red" then
            ...
            IncreaseIfBelowLimit(Red_payload_Attack_SEAD)
            ...
            IncreaseIfBelowLimit(Red_payload_Attackhelo_CAS)
            IncreaseIfBelowLimit(Red_payload_Attackhelo_Trans)
        ...
        if Coalition == "Blue" then
            ...
            N2  = GetPayloadSafe(Blue_payload_Fighter_CAS)
            N3  = GetPayloadSafe(Blue_payload_LtFighter_AA)
            N4  = GetPayloadSafe(Blue_payload_LtFighter_CAS)
            N5  = GetPayloadSafe(Blue_payload_LtFighter_SEAD)
            N6  = GetPayloadSafe(Blue_payload_Attack_CAS)
            N7  = GetPayloadSafe(Blue_payload_Attack_SEAD)
            N8  = GetPayloadSafe(Blue_payload_helo_Trans)
            N9  = GetPayloadSafe(Blue_payload_helo_CAS)
            N10 = GetPayloadSafe(Blue_payload_Attackhelo_CAS)
            N11 = GetPayloadSafe(Blue_payload_Awacs)
        elseif Coalition == "Red" then
            N1  = GetPayloadSafe(Red_payload_Fighter_AA)
            N2  = GetPayloadSafe(Red_payload_LTFighter_CAS)
            N3  = GetPayloadSafe(Red_payload_LtFighter_AA)
            N4  = GetPayloadSafe(Red_payload_Attack_SEAD)
            N5  = GetPayloadSafe(Red_payload_Attack_CAS)
            N6  = GetPayloadSafe(Red_payload_helo_Trans)
            N7  = GetPayloadSafe(Red_payload_helo_CAS)
            N8  = GetPayloadSafe(Red_payload_Attackhelo_CAS)
            N9  = GetPayloadSafe(Red_payload_Attackhelo_Trans)
            N10 = GetPayloadSafe(Red_payload_Awacs)
        else
            env.info("Coalition not found")
        end
        ...
        env.info(string.format(
            "Payloads available after production at %s: Fighter_AA=%d, Fighter_CAS=%d, LtFighter_AA=%d, LtFighter_CAS=%d, LtFighter_SEAD=%d, Attack_CAS=%d, Attack_SEAD=%d, heloTrans=%d, heloCAS=%d, AttackheloCAS=%d, AWACS=%d",
            warehouseName, N1 or 0, N2 or 0, N3 or 0, N4 or 0, N5 or 0, N6 or 0, N7 or 0, N8 or 0, N9 or 0, N10 or 0, N11 or 0
        ))
        if Coalition == "Blue" then 
        local airfieldName = warehouseName:gsub("^warehouse_", "")
        ...
        local Sqn4 = airwing:GetSquadron("Blue Transport Helo Squadron "..airfieldName)
        local Sqn5 = airwing:GetSquadron("Blue Attack Helo Squadron "..airfieldName)
        ...
        local Sqn4 = airwing:GetSquadron("Red Transport Helo Squadron "..airfieldName)
        local Sqn5 = airwing:GetSquadron("Red Attack Helo Squadron "..airfieldName)
...
local Blueawacs = AWACS:New("Darkstar",BlueAwacsAirwing,"blue"    ,AIRBASE:FindByName(BlueAwacsAirfieldName),"CAP_Zone_SE-3",ZONE:FindByName("Bulls"),"CAP_Zone_SE-3",255,radio.modulation.AM )
...
local Redawacs = AWACS:New("Magic",RedAwacsAirwing,"red",AIRBASE:FindByName(BlueAwacsAirfieldName),"CAP_Zone_Mid-1",ZONE:FindByName("Bulls"),"CAP_Zone_Mid-1",245,radio.modulation.AM )
...
function CreateCAPZoneForPlayer(playerGroup, zoneObject, label)
    ...
    -- Determine spawn template (50/50 chance)
    local groupTemplate
    if coalitionSide == coalition.side.BLUE then
        drawfor = 2
        if math.random(0,1) == 0 then
            groupTemplate = Blue_Fighter.."_AA"
        else
            groupTemplate = Blue_LT_Fighter.."_AA"
        end
    elseif coalitionSide == coalition.side.RED then
        drawfor = 1
        if math.random(0,1) == 0 then
            groupTemplate = Red_Fighter.."_AA"
        else
            groupTemplate = Red_LT_Fighter.."_AA"
        end
    end

    if not groupTemplate then
        MESSAGE:New("Internal error: No group template found for CAP spawn.", 10):ToGroup(playerGroup)
        env.info("ERROR: groupTemplate is nil in CreateCAPZoneForPlayer!")
        return
    end

    -- Find nearest friendly airbase
    local friendlyAirfields = (coalitionSide == coalition.side.BLUE) and blueAirfields or redAirfields
    local playerCoord = playerGroup:GetCoordinate()
    local nearestAirbase = nil
    local nearestDist = math.huge

    for _, airfieldName in ipairs(friendlyAirfields) do
        local airbase = AIRBASE:FindByName(airfieldName)
        if airbase then
    ...
    if not nearestAirbase then
        MESSAGE:New("No friendly airbase found for CAP spawn!", 10):ToGroup(playerGroup)
        env.info("ERROR: No friendly airbase found for CAP spawn.")
        return
    end

    local groupName = "PlayerCAP_" .. playerName .. "_" .. math.random(1000,9999)
    local capSpawner = SPAWN:NewWithAlias(groupTemplate, groupName)
        :InitGrouping(2)

    capSpawner:OnSpawnGroup(function(spawnedGroup)
        local flightGroup = FLIGHTGROUP:New(spawnedGroup)
        -- Build the CAP zone
        local capZoneName, capZoneRadius, coord
        if zoneObject then
            capZoneName = "CAP_Zone_" .. (label or "Player") .. "_" .. playerName
            capZoneRadius = 15000
        ...
        --draw the zone for 30 minutes for the requesting coalition
        capZone:DrawZone(drawfor, {0,1,0}, 1, {0,1,0}, .15, 4)
        capZone:UndrawZone(60*30)
        local CapRequest = AUFTRAG:NewCAP(capZone, 30000, 400)
            :SetPriority(20, 1, 3)
            :SetRequiredAssets(2, 2)
            :SetDuration(60*30)
        -- After creating the AUFTRAG and before assigning it to the flight group/chief:

        flightGroup:AddMission(CapRequest)
        -- Assign the mission to the correct chief
        if coalitionSide == coalition.side.BLUE then
            
            MESSAGE:New("Blue CAP flight spawned and assigned to your requested zone!", 10):ToGroup(playerGroup)
        elseif coalitionSide == coalition.side.RED then
            
            MESSAGE:New("Red CAP flight spawned and assigned to your requested zone!", 10):ToGroup(playerGroup)
        end
    ...
    capSpawner:SpawnAtAirbase(nearestAirbase, SPAWN.Takeoff.Air, 2000)
end
...
function CreateCASZoneForPlayer(playerGroup, zoneObject, label)
    if not playerGroup or not playerGroup:IsAlive() then
        MESSAGE:New("Your group is not alive or not found!", 10):ToGroup(playerGroup)
        return
    end
    ...
    if CoalitionReqCount[coalitionSide] >= MAX_REQ_PER_COALITION then
        MESSAGE:New("Your coalition has reached the maximum number of CAS missions!", 10):ToGroup(playerGroup)
        return
    end
    ...
    if coalitionSide == coalition.side.BLUE then
        drawfor = 2
        if math.random(0,1) == 0 then
            groupTemplate = Blue_Attack.."_CAS"
        else
            groupTemplate = Blue_LT_Fighter.."_CAS"
        end
    elseif coalitionSide == coalition.side.RED then
        drawfor = 1
        if math.random(0,1) == 0 then
            groupTemplate = Red_Attack.."_CAS"
        else
            groupTemplate = Red_LT_Fighter.."_CAS"
        end
    end

    if not groupTemplate then
        MESSAGE:New("Internal error: No group template found for CAS spawn.", 10):ToGroup(playerGroup)
        env.info("ERROR: groupTemplate is nil in CreateCASZoneForPlayer!")
        return
    end
    ...
    if not nearestAirbase then
        MESSAGE:New("No friendly airbase found for CAS spawn!", 10):ToGroup(playerGroup)
        env.info("ERROR: No friendly airbase found for CAS spawn.")
        return
    end

    local groupName = "PlayerCAS_" .. playerName .. "_" .. math.random(1000,9999)
    local casSpawner = SPAWN:NewWithAlias(groupTemplate, groupName)
        :InitGrouping(2)

    casSpawner:OnSpawnGroup(function(spawnedGroup)
        local flightGroup = FLIGHTGROUP:New(spawnedGroup)
        -- Build the CAS zone
        local casZoneName, casZoneRadius, coord
        if zoneObject then
            casZoneName = "CAS_Zone_" .. (label or "Player") .. "_" .. playerName
            casZoneRadius = 15000
            coord = zoneObject:GetCoordinate()
        else
            casZoneName = "CAS_Zone_Player_" .. playerName
            casZoneRadius = 20000
            coord = playerGroup:GetCoordinate()
        end
        local casZone = ZONE_RADIUS:New(casZoneName, coord:GetVec2(), casZoneRadius)
        --draw the zone for 30 minutes for the requesting coalition
        casZone:DrawZone(drawfor, {1,1,0}, 1, {1,1,0}, .15, 4)
        casZone:UndrawZone(60*30)
        local CasRequest = AUFTRAG:NewCAS(casZone, 5000, 350)
            :SetPriority(20, 1, 3)
            :SetRequiredAssets(2, 2)
            :SetDuration(60*30)

        flightGroup:AddMission(CasRequest)
        -- Assign the mission to the correct chief
        if coalitionSide == coalition.side.BLUE then
            
            MESSAGE:New("Blue CAS flight spawned and assigned to your requested zone!", 10):ToGroup(playerGroup)
        elseif coalitionSide == coalition.side.RED then
            
            MESSAGE:New("Red CAS flight spawned and assigned to your requested zone!", 10):ToGroup(playerGroup)
        end

        PlayerRequests[playerName] = PlayerRequests[playerName] + 1
        CoalitionReqCount[coalitionSide] = CoalitionReqCount[coalitionSide] + 1
        table.insert(PlayerMissions[playerName], {type="CAS", id=casZoneName, label=label or casZoneName})
    end)

    casSpawner:SpawnAtAirbase(nearestAirbase, SPAWN.Takeoff.Air, 2000)
end
    ...
    MENU_GROUP_COMMAND:New(playerGroup, "Refresh Nearest Enemy airfields", A2GMenu, function()
        ...
        for i = 1, math.min(5, #airfieldDistances) do
            ...
            MENU_GROUP_COMMAND:New(playerGroup, "Request CAS at " .. airfieldName, dynamicMenu, function()
                -- Use the airbase's zone for the CAS mission
                local airbase = AIRBASE:FindByName(airfieldName)
                if airbase then
                    CreateCASZoneForPlayer(playerGroup, airbase:GetZone(), airfieldName)
                else
...
function RequestEscortMission(playerGroup)
    ...
    if not groupTemplate then
        MESSAGE:New("Internal error: No group template found for ESCORT spawn.", 10):ToGroup(playerGroup)
        env.info("ERROR: groupTemplate is nil in RequestEscortMission!")
        return
    end
    ...
    if not nearestAirbase then
        MESSAGE:New("No friendly airbase found for ESCORT spawn!", 10):ToGroup(playerGroup)
        env.info("ERROR: No friendly airbase found for ESCORT spawn.")
        return
    end

    local groupName = "PlayerESCORT_" .. playerName .. "_" .. math.random(1000,9999)
    local escortSpawner = SPAWN:NewWithAlias(groupTemplate, groupName)
        :InitGrouping(2)

    escortSpawner:OnSpawnGroup(function(spawnedGroup)
        local flightGroup = FLIGHTGROUP:New(spawnedGroup)
        -- Create the ESCORT AUFTRAG targeting the player group
        ...
        flightGroup:AddMission(escortMission)

        -- Notify player
        if coalitionSide == coalition.side.BLUE then
            MESSAGE:New("Blue escort flight spawned and assigned to your group!", 10):ToGroup(playerGroup)
        elseif coalitionSide == coalition.side.RED then
            MESSAGE:New("Red escort flight spawned and assigned to your group!", 10):ToGroup(playerGroup)
        end
    ...
    escortSpawner:SpawnAtAirbase(nearestAirbase, SPAWN.Takeoff.Air, 2000)
end
...
function AddMenuForAllPlayers()
    ...
    playerSet:ForEachGroup(function(group)
        if group:IsPlayer() then
            ...
            -- CAS mission over player
            MENU_GROUP_COMMAND:New(group, "Request CAS (Over Me)", A2GMenu, function()
                CreateCASZoneForPlayer(group, nil, "Player")
            end)
            -- Dynamic nearest enemy airfields for CAS
            AddDynamicNearestCASMenu(group, A2GMenu)
            -- Dynamic nearest airfields for CAP
            AddDynamicNearestCAPMenus(group, A2AMenu)
            -- Active missions menu
...
function ATCGroundOps()
    ...
    for _, airfieldName in ipairs(AirfieldNames) do
        local airbase = AIRBASE:FindByName(airfieldName)
        if airbase then
            local abCategory = airbase:GetCategory()
            if abCategory == Airbase.Category.AIRDROME then
                -- Extra check: ensure airbase has at least one runway
                local runways = nil
                    if airbase.GetRunways and type(airbase.GetRunways) == "function" then
                        runways = airbase:GetRunways()
                    end
                if runways and #runways > 0 then
                    Freq = Freq + 0.50
                    local atc = FLIGHTCONTROL:New(airfieldName, Freq, 0, "C:\\Program Files\\DCS-SimpleRadio-Standalone", 5002)
                    ...
                    atc:Start()
                    ATC_Controllers[airfieldName] = atc
                    env.info("ATC Ground Ops: " .. airfieldName .. " at frequency " .. Freq)
                else
                    env.info("Skipping ATC: No runways found for " .. tostring(airfieldName))
                end
            else
                env.info("Skipping ATC for non-airdrome: " .. airfieldName)
            end
        else
            env.info("Skipping ATC: Airbase not found for " .. tostring(airfieldName))
        end
    end
end
