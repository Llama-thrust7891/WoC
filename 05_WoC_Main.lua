...
function Spawn_Near_airbase(GroupTemplate, airfieldName, Inner, Outer, Patrol)
    ...
    if WarehouseFound then
        ...
        Spawnpoint = WarehouseZone:GetRandomClearZoneCoordinate(20, 20) -- 20m radius 20 attempts
        env.info("Spawn point set near warehouse at " .. airfieldName)
    else
        -- Default to airbase zone if no warehouse is found
        Spawnpoint = SpawnZone:GetRandomCoordinate(Inner, Outer, land.SurfaceType.LAND)  --fallback to random point that is Land
...
function SpawnWarehouse(airfieldName, warehouseName, coalitionSide)
    ...
    local SpawnZone = airbase:GetZone()
    local Spawnpoint = nil
    local MaxAttempts = 50  -- Try up to 50 locations if obstructed
    local Attempts = 0

    repeat
        Spawnpoint = SpawnZone::GetRandomClearZoneCoordinate(100,1)
...
--Simulated asset production code in "09_WoC_AssetProd.lua"
